...
app.get("/", async (request, response) => {
  let { search, limit, offset, month } = request.query;
  const finalData = {
    all_transactions: [],
    statistics: {},
    bar_chart: {},
  ...
  finalData.all_transactions = productArray;

  const transactionArray = await db.all(getTransactionData);

  let totalSale = 0;
  let totalSold = 0;
  let totalNotSold = 0;
  transactionArray.map((eachData) => {
    totalSale += eachData.price;
    if (eachData.sold === "true") {
      totalSold += 1;
    } else {
      totalNotSold += 1;
    }
  });
  finalData.statistics["total_sale"] = totalSale;
  finalData.statistics["total_sold_item"] = totalSold;
  finalData.statistics["total_not_sold_item"] = totalNotSold;

  let barChart = {
    below_100: 0,
    between_101_to_200: 0,
    between_201_to_300: 0,
    between_301_to_400: 0,
    between_401_to_500: 0,
    between_501_to_600: 0,
    between_601_to_700: 0,
    between_701_to_800: 0,
    between_801_to_900: 0,
    above_900: 0,
  };
  transactionArray.map((eachData) => {
    if (eachData.price < 101) {
      barChart.below_100 += 1;
    } else if (eachData.price > 100 && eachData.price < 201) {
      barChart.between_101_to_200 += 1;
    } else if (eachData.price > 200 && eachData.price < 301) {
      barChart.between_201_to_300 += 1;
    } else if (eachData.price > 300 && eachData.price < 401) {
      barChart.between_301_to_400 += 1;
    } else if (eachData.price > 400 && eachData.price < 501) {
      barChart.between_401_to_500 += 1;
    } else if (eachData.price > 500 && eachData.price < 601) {
      barChart.between_501_to_600 += 1;
    } else if (eachData.price > 600 && eachData.price < 701) {
      barChart.between_601_to_700 += 1;
    } else if (eachData.price > 700 && eachData.price < 801) {
      barChart.between_701_to_800 += 1;
    } else if (eachData.price > 800 && eachData.price < 901) {
      barChart.between_801_to_900 += 1;
    } else {
      barChart.above_900 += 1;
    }
  });
  finalData.bar_chart = barChart;

  let pieChart = {};
  transactionArray.map((eachData) => {
    pieChart[eachData.category] = 0;
  });
  transactionArray.map((eachData) => {
    pieChart[eachData.category] += 1;
  });
  finalData.pie_chart = pieChart;
